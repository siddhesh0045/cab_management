...
router.get('/view_cars', (req, res) => {
    ...
    con.query(query, values, (error, results) => {
      if (error) {
        if (error.code === 'ER_DUP_ENTRY') {
          ...
          res.status(400).json({ error: 'The registration number already exists' });
        } else {
          // Handle other errors
          console.error('Error inserting data into the "cars" table:', error);
          res.status(500).json({ error: 'Internal server error' });
  ...
  //   }catch{
  //     res.send("Car Id you are looking is not available")
  //   }
  //   });
  // });
  router.post("/updatecar", (req, res) => {
    ...
    con.query(getCarQuery, [carID], (selectError, selectResults) => {
      if (selectError) {
        ...
        if (newID !== "-1") {
          ...
          con.query( increaseCountQuery,[newID, typeID, newID, typeID],(error, results) => {
              if (error) {
                ...
                res
                  .status(200)
                  .json({ message: "Car counts updated successfully for types table" });
              }
            }
          );
        }
        ...
        const updatedCar = {
          type_id:
            req.body.type_id !== "-1" ? req.body.type_id : existingCar.type_id,
          registration_number:
            req.body.UReg_number !== " "
              ? req.body.UReg_number
              : existingCar.registration_number,
          color:
            req.body.Ucolor !== " " ? req.body.Ucolor : existingCar.color,
          present:
            req.body.present !== "-1" ? req.body.present : existingCar.present,
        };
