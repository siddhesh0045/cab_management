...
//add driver
  router.post("/add", (req, res) => {
    ...
    con.query(query, values, (error, results) => {
      if (error) {
        ...
        con.query(updateQuery, updateValues, (updateError, updateResults) => {
          if (updateError) {
            console.error('Error updating data in the "driver" table:', updateError);
          } else {
            console.log('Count value updated successfully in the "driver" table');
          }
        });
      }
    });
  ...
  router.post("/delete", (req, res) => {
    ...
    con.query(getTypeIDQuery, [driverId], (selectError, selectResults) => {
      if (selectError) {
        ...
        const language = selectResults[0].lan;
        const updateQuery =
          "UPDATE driver_type SET count = count - 1 WHERE d_language = ? and count > 0" ;
        con.query(updateQuery, [language], (updateError, updateResults) => {
          if (updateError) {
            console.error("Error updating count in driver_type table:", updateError);
            res.status(500).send("Error deleting driver");
          } else {
            console.log("Count value updated successfully in driver_type table");
  
            // Perform the deletion query
            con.query( "DELETE FROM drivers WHERE id = ?",[driverId],(error, results) => {
                if (error) {
                  console.error("Error deleting driver:", error);
                  res.status(500).send("Error deleting driver");
                } else {
  ...
  router.post("/update", (req, res) => {
    ...
    // Retrieve the existing driver details from the database
    const getDriverQuery = "SELECT * FROM drivers WHERE id = ?";
    con.query(getDriverQuery, [driverID], (selectError, selectResults) => {
      if (selectError) {
        console.error("Error retrieving Driver details:", selectError);
        res.status(500).send("Error updating Drivers");
      } else if (selectResults.length === 0) {
        console.error("Driver not found");
        res.status(404).sendFile(path.join(__dirname, "views/server_error.html"));
      } else {
        const currentDriver = selectResults[0];
        const currlan = currentDriver.lan;
        const newlan = req.body.ulanguage;
            ...
            (error, results) => {
              if (error) {
                ...
                res
                  .status(200)
                  .json({
                    message: "Driver counts updated successfully for types table",
                  });
              }
            }
          );
        ...
        const updatedDriver = {
          dr_name:
            req.body.uname !== " " ? req.body.uname : currentDriver.dr_name,
          licence:
            req.body.ulicence !== " "
              ? req.body.ulicence
              : currentDriver.licence,
          lan:
            req.body.ulanguage !== "-1" ? req.body.ulanguage : currentDriver.lan,
          present:
            req.body.upresent !== "-1"
              ? req.body.upresent
